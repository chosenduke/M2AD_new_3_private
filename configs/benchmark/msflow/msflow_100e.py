...
class cfg(cfg_common, cfg_dataset_default, cfg_model_msflow):

	def __init__(self):
		cfg_common.__init__(self)
		cfg_dataset_default.__init__(self)
		cfg_model_msflow.__init__(self)
		...
		# ==> trainer
		self.trainer.name = 'MSFlowTrainer'
		...
		self.loss.loss_terms = [
			dict(type='CDOLoss', name='msflow_loss', gamma=2, OOM=True),
		] ### TODO
		...
		self.logging.log_terms_train = [
			...
			dict(name='msflow_loss', suffixes=[''], fmt=':>5.3f', add_name='avg'),
		]
