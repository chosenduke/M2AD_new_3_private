...
def setup_cfg(cfg, setup, cls):
    cfg.setup = setup
    cfg.train_data.cls_names = cls
    cfg.test_data.cls_names = cls

    if setup == 'M2AD-Synergy':
        cfg.metrics = cfg.m2ad_synergy_metrics
        cfg.evaluator.kwargs = dict(metrics=cfg.metrics, pooling_ks=None, max_step_aupro=100,
                                     use_adeval=cfg.use_adeval)
    elif setup == 'M2AD-Invariant':
        cfg.metrics = cfg.m2ad_invariant_metrics
        cfg.evaluator.kwargs = dict(metrics=cfg.metrics, pooling_ks=None, max_step_aupro=100,
                                    use_adeval=cfg.use_adeval)
    return cfg
