...
class Evaluator(object):
    ...
    def execute_img_instance(self, results, cls_name):
        ...
        for metric in self.metrics:
            ...
            elif metric.startswith('mF1_img_px'):
        ...
        return metric_results, metric_str

    def execute_view_instance(self, results, cls_name):
        idxes = results['view_instance_cls_names'] == cls_name
        gt_px = results['view_instance_masks'][idxes]
        pr_px = results['view_instance_anomaly_maps'][idxes]
        gt_sp = results['view_instance_anomalys'][idxes]
        pr_sp = results['view_instance_anomaly_scores'][idxes]
        ...
        for metric in self.metrics:
            t0 = get_timepc()
            if metric.startswith('AUROC_view_sp'):
                auroc_sp = roc_auc_score(gt_sp, pr_sp)
                metric_results[metric] = auroc_sp
            elif metric.startswith('AUROC_view_px'):
                if not self.use_adeval:
                    auroc_px = roc_auc_score(gt_px.ravel(), pr_px.ravel())
                    metric_results[metric] = auroc_px
                else:
                    metric_results[metric] = metrics['p_auroc']
            elif metric.startswith('AUPRO_view'):
                if not self.use_adeval:
                    aupro_px = self.cal_pro_score(gt_px, pr_px, max_step=self.max_step_aupro, mp=self.mp)
                    metric_results[metric] = aupro_px
                else:
                    metric_results[metric] = metrics['p_aupro']
            elif metric.startswith('AP_view_sp'):
                ap_sp = average_precision_score(gt_sp, pr_sp)
                metric_results[metric] = ap_sp
            elif metric.startswith('AP_view_px'):
                if not self.use_adeval:
                    ap_px = average_precision_score(gt_px.ravel(), pr_px.ravel())
                    metric_results[metric] = ap_px
                else:
                    metric_results[metric] = metrics['p_aupr']
            elif metric.startswith('mF1_view_sp'):
                precisions, recalls, thresholds = precision_recall_curve(gt_sp, pr_sp)
                f1_scores = (2 * precisions * recalls) / (precisions + recalls)
                best_f1_score = np.max(f1_scores[np.isfinite(f1_scores)])
                best_f1_score_index = np.argmax(f1_scores[np.isfinite(f1_scores)])
                best_threshold = thresholds[best_f1_score_index]
                metric_results[metric] = best_f1_score
            elif metric.startswith('mF1_view_px'):
                # F1_px equals Dice_px
                score_l, score_h, score_step = 0.0, 1.0, 0.05
                gt = gt_px.astype(np.bool_)
                metric_scores = []
    ...
    def execute_object_instance(self, results, cls_name):
        idxes = results['object_instance_cls_names'] == cls_name
        gt_sp = results['object_instance_anomalys'][idxes]
        pr_sp = results['object_instance_anomaly_scores'][idxes]

        metric_str = f'==> Metric Time for {cls_name:<15}: '
        metric_results = {}
        for metric in self.metrics:
            t0 = get_timepc()
            if metric.startswith('AUROC_obj_sp'):
                auroc_sp = roc_auc_score(gt_sp, pr_sp)
                metric_results[metric] = auroc_sp
            elif metric.startswith('AP_obj_sp'):
                ap_sp = average_precision_score(gt_sp, pr_sp)
                metric_results[metric] = ap_sp
            elif metric.startswith('mF1_obj_sp'):
                precisions, recalls, thresholds = precision_recall_curve(gt_sp, pr_sp)
                f1_scores = (2 * precisions * recalls) / (precisions + recalls)
                best_f1_score = np.max(f1_scores[np.isfinite(f1_scores)])
                best_f1_score_index = np.argmax(f1_scores[np.isfinite(f1_scores)])
    ...
    def run(self, results, cls_name, setup, logger=None):
        if setup == 'M2AD-Synergy':
            view_metric_results, view_metric_str = self.execute_view_instance(results, cls_name)
            object_metric_results, object_metric_str = self.execute_object_instance(results, cls_name)

            metric_results = {}
            metric_results.update(view_metric_results)
            metric_results.update(object_metric_results)

            metric_str = view_metric_str + object_metric_str
        elif setup == 'M2AD-Invariant':
            img_metric_results, img_metric_str = self.execute_img_instance(results, cls_name)

            metric_results = {}
            metric_results.update(img_metric_results)
            metric_str = img_metric_str
        else:
