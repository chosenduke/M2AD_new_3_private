...
def vis_rgb_gt_amp(img_paths, imgs, cls_name, img_masks, anomaly_maps_, method, root_out):
    ...
    for idx, (img_path, img, img_mask, anomaly_map, cls) in enumerate(zip(img_paths, imgs, img_masks, anomaly_maps, cls_name)):
        parts = img_path.split('/')
        needed_parts = parts[-3:-1]
        specific_root = "_".join(needed_parts)
        # specific_root = '/'.join(needed_parts)
        img_num = parts[-1].split('.')[0]

        out_dir = f'{root_out}/{method}/{cls}'
...
def plot_two_pair_distri(a_before=None, n_before=None, a_after=None, n_after=None,
                         ...
                         save_name='temp.png', category='皮革'):
