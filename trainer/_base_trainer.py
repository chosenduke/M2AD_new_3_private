...
from util.m2ad_utils import m2ad_fuse
import pickle
...
class BaseTrainer():
    ...
    def test(self, vis=True):
        ...
        obj_names, cls_names, illumination_list, view_list, imgs_masks, \
        image_anomalys, object_anomalys, anomaly_scores, anomaly_maps = [], [], [], [], [], [], [], [], []
        ...
        while batch_idx < test_length:
            ...
            image_anomalys.append(self.image_anomaly.cpu().numpy().astype(int))
            object_anomalys.append(self.object_anomaly.cpu().numpy().astype(int))

            obj_names.append(np.array(self.object_name))
            cls_names.append(np.array(self.cls_name))
            illumination_list.append(np.array(self.illumination))
            view_list.append(np.array(self.view))
            ...
            if self.cfg.vis and vis:
                ...
                vis_rgb_gt_amp(self.img_path, self.imgs, self.cls_name, self.imgs_mask.cpu().numpy().astype(int),
        ...
        results = dict(obj_names=obj_names, cls_names=cls_names, illumination_list=illumination_list,
                       view_list=view_list, imgs_masks=imgs_masks, image_anomalys=image_anomalys,
                       object_anomalys=object_anomalys, anomaly_scores=anomaly_scores, anomaly_maps=anomaly_maps)
        ...
        # results_path = os.path.join(root_out, f'{self.cfg.train_data.cls_names}_results.pkl')
        # with open(results_path, 'wb') as f:
        #     pickle.dump(results, f)

        if self.cfg.setup == 'M2AD-Synergy':
            fused_results = m2ad_fuse(**results, fuse='add')
            log_msg(self.logger, f"\n #objects: {fused_results['object_instance_names'].shape[0]}, "
                                 f"#view instances: {fused_results['view_instance_names'].shape[0]}")
        else:
            fused_results = results
            log_msg(self.logger, f"\n #images: {fused_results['obj_names'].shape[0]}")

        msg = {}
        for idx, cls_name in enumerate(self.test_cls_names):
            metric_results = self.evaluator.run(fused_results, cls_name, self.cfg.setup, self.logger)
